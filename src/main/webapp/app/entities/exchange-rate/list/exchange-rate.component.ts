...
import { EntityArrayResponseType, ExchangeRateService } from '../service/exchange-rate.service';
...
export class ExchangeRateComponent implements OnInit {
  ...
  trackId = (item: IExchangeRate): number => this.exchangeRateService.getExchangeRateIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.exchangeRates.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IExchangeRate[] | null): IExchangeRate[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.exchangeRateService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }

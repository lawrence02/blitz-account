...
type BudgetFormGroupContent = {
  ...
  accountId: FormControl<BudgetFormRawValue['accountId']>;
  amount: FormControl<BudgetFormRawValue['amount']>;
};
...
export class BudgetFormService {
  createBudgetFormGroup(budget: BudgetFormGroupInput = { id: null }): BudgetFormGroup {
    ...
    return new FormGroup<BudgetFormGroupContent>({
      ...
      accountId: new FormControl(budgetRawValue.accountId),
      amount: new FormControl(budgetRawValue.amount, {
        validators: [Validators.required],
      }),
    });
  }

...
import { EntityArrayResponseType, JournalService } from '../service/journal.service';
...
export class JournalComponent implements OnInit {
  ...
  trackId = (item: IJournal): number => this.journalService.getJournalIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.journals.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IJournal[] | null): IJournal[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.journalService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }

...
import { EntityArrayResponseType, QuoteService } from '../service/quote.service';
...
export class QuoteComponent implements OnInit {
  ...
  trackId = (item: IQuote): number => this.quoteService.getQuoteIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.quotes.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IQuote[] | null): IQuote[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.quoteService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
